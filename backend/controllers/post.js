...
exports.likePost = (req, res, next) => {
  Post.updateOne(
    { _id: req.params.id },
    { $addToSet: { likes: req.body.userId }, _id: req.params.id },
  )
    .then(() =>
      res.status(200).json({
        message: "Le user a bien été ajouté à la liste de likes du post",
      }),
    )
    .catch((error) => res.status(400).json({ error }));
};
...
exports.unlikePost = (req, res, next) => {
  Post.updateOne(
    { _id: req.params.id },
    { $pull: { likes: req.body.userId }, _id: req.params.id },
  )
    .then(() =>
      res.status(200).json({
        message: "Le user a bien été retiré de la liste de likes du post",
      }),
    )
    .catch((error) => res.status(400).json({ error }));
};
